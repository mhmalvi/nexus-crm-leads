    ...
    public function dataScraper(Request $request)
    {
        if ($request->bearerToken()) {
            ...
            if ($flag_receive == 1) {
                ...
                $url = "https://graph.facebook.com/v18.0/me?fields=id,name,adaccounts{campaigns.limit(10000){name,start_time,stop_time,status,ads{leads.limit(1000000)}},business_name}&access_token=" . $accessToken;
                                                                ...
                                                                if (count($tempArray['lead'][$lead['id']]['data']) > 0) {
                                                                    foreach ($tempArray['lead'][$lead['id']]['data'] as $fieldValue) {
                                                                        ...
                                                                        if (strlen(stristr($fieldValue['name'], "qualification_are_you")) > 0) {
                                                                            ...
                                                                            $courseCodeArray = explode('_', iconv('utf-8', 'ascii//IGNORE', $this->_cleanString($fieldValue['values'][0])));

                                                                            // $leadDetailsInfo['lead'][$lead['id']]['course_title']= $this->_cleanString(str_replace("_", " ",iconv('utf-8', 'ascii//TRANSLIT',$fieldValue['values'][0])));

                                                                            $leadDetailsInfo['lead'][$lead['id']]['course_title'] = $this->_cleanString(str_replace("_", " ", iconv('utf-8', 'ascii//IGNORE', $fieldValue['values'][0])));

                                                                            foreach ($courseCodeArray as $courseCodeString) {
                                                                                $isThereNumber = false;
                                                                                // $courseCodeString = iconv('utf-8', 'ascii//TRANSLIT', $courseCodeString);
                                                                                $courseCodeString = iconv('utf-8', 'ascii//IGNORE', $courseCodeString);
