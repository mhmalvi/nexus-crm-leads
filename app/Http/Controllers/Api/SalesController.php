...
class SalesController extends Controller
{
    ...
    public function lead_list_in_sales(Request $request, $sales_id, $company_id)
    {
        ...
        //     $flag_receive = $flag['data'];
        //     if ($flag_receive == 1) {
        $lead_list = LeadDetails::join('courses_info', 'lead_details.course_id', '=', 'courses_info.id')->where('lead_details.client_id', $company_id)->where('lead_details.sales_user_id', $sales_id)->orderBy('lead_apply_date', 'desc')->get();
        if ($lead_list) {
            return response()->json([
                'message'    => 'success',
                'status' => 200,
